(function($) {
	...
	function RemoteModel() {
		...
		function onSuccess(resp) {
			/*var from = 0, to = 0 + resp.length;

			for (var i = 0; i < resp.length; i++) {
				data[from + i] = resp[i].text; 
				data[from + i].index = from + i;
			}*/
			var tweets = resp.split("\n\n");
			data.length = parseInt(resp.length);
			var from = 0, to = 0 + tweets.length;
			for (var i = 0; i < tweets.length; i++) {
				data[from + i] = tweets[i]; 
				data[from + i].index = from + i;
			}
